...
test('write 4.buffer', function(t) {
  var writer = new BerWriter();
  ...
  for (var i = 13, j = 0; i < ber.length && j < buf.length; i++, j++) {
    t.equal(ber[i], buf[j], '4.buffer contents not identical');
  }
  t.end();
});
...
test('resize internal 4.buffer', function(t) {
...
test('LDAP bind message', function(t) {
  ...
  t.ok(ber);
  t.equal(ber.length, 35, 'wrong length (4.buffer)');
  t.equal(ber[0], 0x30, 'wrong tag');
